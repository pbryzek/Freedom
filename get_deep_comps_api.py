...
class APIGetDeepCompsRequest(BaseHomeAPI):
    ...
    def parse_comp(self, node, valid_tier):

        tier_dict = switches.TIER_VALUES[valid_tier]
        sqft_param = tier_dict[switches.TIER_SQFT_KEY]
        dist_param = tier_dict[switches.TIER_DISTANCE_KEY]
        sold_param = tier_dict[switches.TIER_SOLD_KEY]
        ...
        if miles > dist_param:
            handle_err_msg("Comp property is " + str(miles) + " away from the principal and max allowed miles = " + str(dist_param) + " skipping")
            return

        if self.principal_sqfootage.strip() != '':

            min_sqfootage = int(self.principal_sqfootage) * (1-sqft_param)
            max_sqfootage = int(self.principal_sqfootage) * (1+sqft_param)
        ...
        if self.solddate.strip() != '':
            ...
            time_diff_h = time_diff_m / 60
            time_diff_d = time_diff_h / 24
            time_diff_mon = time_diff_d / 30

            if sold_param < time_diff_mon:
                handle_err_msg("Sold date: " + str(self.solddate) + " is " + str(time_diff_mon) + " months diff which is > than the max of " + str(sold_param) + " skipping")
                return
    ...
    def parse_comparables(self, node):
        num_comps = 0
        #Do the analysis for each tier
        for valid_tier in switches.VALID_TIERS:
            self.tier = valid_tier

            for child in node:
                tag = child.tag
                if tag == tags.TAG_COMP:
                    num_comps += 1
                    self.parse_comp(child, valid_tier)

            #If we found enough valid comps then we are done
            if len(self.homes) >= switches.MIN_NUM_COMPS:
                break
            #Otherwise do the analysis for the next tier
            num_comps = 0
            self.homes[:] = []
